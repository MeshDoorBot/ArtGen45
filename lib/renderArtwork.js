...
function timeRangeParts(text) {
  const parts = String(text).split(' → ');
  return parts.length === 2 ? parts : null;
}

function measureTimeRange(ctx, text, fontSize) {
  const parts = timeRangeParts(text);
  if (!parts) return ctx.measureText(text).width;
  const gap = fontSize * 0.28;
  const arrowWidth = fontSize * 0.7;
  return ctx.measureText(parts[0]).width + gap + arrowWidth + gap + ctx.measureText(parts[1]).width;
}

function drawTimeRangeOpticalCenter(ctx, text, x, centerY, fontSize) {
  const parts = timeRangeParts(text);
  if (!parts) {
    drawTextOpticalCenter(ctx, text, x, centerY);
    return;
  }
  ...
  const gap = fontSize * 0.28;
  const arrowWidth = fontSize * 0.7;
  const totalWidth = measureTimeRange(ctx, text, fontSize);
  let nextX = align === 'center' ? x - totalWidth / 2 : x;
  if (align === 'right' || align === 'end') nextX = x - totalWidth;

  ctx.textAlign = 'left';
  drawTextOpticalCenter(ctx, parts[0], nextX, centerY);
  nextX += ctx.measureText(parts[0]).width + gap;

  const arrowY = centerY;
  const arrowHead = Math.max(3, fontSize * 0.16);
  ctx.save();
  ctx.strokeStyle = ctx.fillStyle;
  ctx.lineWidth = Math.max(1.5, fontSize * 0.075);
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  ctx.beginPath();
  ctx.moveTo(nextX, arrowY);
  ctx.lineTo(nextX + arrowWidth, arrowY);
  ctx.moveTo(nextX + arrowWidth - arrowHead, arrowY - arrowHead);
  ctx.lineTo(nextX + arrowWidth, arrowY);
  ctx.lineTo(nextX + arrowWidth - arrowHead, arrowY + arrowHead);
  ctx.stroke();
  ctx.restore();

  nextX += arrowWidth + gap;
  drawTextOpticalCenter(ctx, parts[1], nextX, centerY);
  ctx.textAlign = align;
}
...
function drawTracklistArtwork(ctx, format, info, lines) {
  ...
  meta.forEach((text, index) => {
    ...
    const pillW = (text === timeStr ? measureTimeRange(ctx, text, format.label === 'story' ? 30 : 24) : ctx.measureText(text).width) + 32;
    drawPill(ctx, nextX, metaY, pillW, metaH);
    ctx.fillStyle = '#050505';
    ctx.textAlign = 'center';
    if (text === timeStr) {
      drawTimeRangeOpticalCenter(ctx, text, nextX + pillW / 2, metaY + metaH / 2, format.label === 'story' ? 30 : 24);
    } else {
...
export async function renderArtwork({ canvas, image, logo, info, formatKey, layoutKey, showPills = true, showLogo = true, tracklist = '', imageTweak = {} }) {
  ...
  infoPills.forEach((text, index) => {
    const textW = text === timeStr ? measureTimeRange(ctx, text, metaFontSize) : ctx.measureText(text).width;
    ...
    if (text === timeStr) {
      drawTimeRangeOpticalCenter(ctx, text, showPills ? nextX + bubbleW / 2 : nextX, dateY + metaPillHeight / 2, metaFontSize);
    } else {
